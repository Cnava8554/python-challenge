...
createFile = os.path.join("Analysis","Election Results.txt")
with open(createFile,"w") as file:
    ...
    file.write("----------------------------")
    file.write("\n")
    file.write(f'{(list(set(candidate_options))[0])}: {RPercent}% {len(Raymond)}')
    file.write("\n")
    file.write(f'{(list(set(candidate_options))[1])}: {DPercent}% {len(Diana)}')
    file.write("\n")
    file.write(f'{(list(set(candidate_options))[2])}: {CPercent}% {len(Charles)}')
    file.write("\n")
    file.write("----------------------------")
    file.write("\n")
    file.write('Winner:')